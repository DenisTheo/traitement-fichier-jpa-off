...
import fr.diginamic.traitement.entities.AllergenEntity;
...
{
    @Override
    public void insertAll(Iterable<Allergen> allergens)
    {
        insert(allergens, ServiceLoader.createEntityManager());
    }

    @Override
    public void insertAll(Iterable<Allergen> allergens, EntityManager manager)
    {
        insert(allergens, manager);
    }

    @Override
    public Set<AllergenEntity> findByName(Iterable<String> names)
    {
        return find(names, ServiceLoader.createEntityManager());
    }

    @Override
    public Set<AllergenEntity> findByName(Iterable<String> names, EntityManager manager)
    {
    ...
    {
        return manager.createQuery("SELECT a FROM AllergenEntity as a WHERE a.name IN :names", AllergenEntity.class).setParameter("names", names).getResultStream().collect(Collectors.toSet());
    }
}
